	...
	public void handelRequest(String[] commandData) {
		...
		case "balance": {
			...
			case "add": {
				...
				this.ACK((T) ("balance " + ((rentalMovieUser) this.userLogin()).getBalance() + " added " + amount));
		...
		case "info": {
			...
			if (commandData.length < 3) {
				...
				if (movie == null) {
					this.ERROR((T) ("request info failed"));
					this.moviesLock.readLock().unlock();
					return;
				}
				String countries = "";
				for (int i = 0; i < movie.getBannedCountries().size(); i++)
					countries = countries + " " + "\"" + movie.getBannedCountries().get(i) + "\"";

				toReturn = "\""+movie.getName()+"\"" + " " + movie.getAvailbleAmount() + " " + movie.getPrice() + countries;
				this.ACK((T) ("info " + toReturn));
				this.moviesLock.readLock().unlock();
			}
		...
		case "addmovie": {
			...
			if (intAmount > 0 & intPrice > 0) {
				Movie movie = new Movie(this.movieDataBase.getHighestId(), movieName, intPrice, bannedCountriesList,
						intAmount, intAmount);
				this.usersLock.readLock().unlock();
				this.moviesLock.readLock().unlock();
				this.moviesLock.writeLock().lock();
